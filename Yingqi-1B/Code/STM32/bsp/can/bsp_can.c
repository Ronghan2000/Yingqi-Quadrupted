...
static void EXTCANAddFilter(CANInstance *_instance){
    ...
    static uint8_t can1_filter_idx = 0, can2_filter_idx = 14; // 8-13给ex can1用,23-27给ex can2用
...
static void CANFIFOxCallback(CAN_HandleTypeDef *_hcan, uint32_t fifox)
{
    ...
    while (HAL_CAN_GetRxFifoFillLevel(_hcan, fifox)) // FIFO不为空,有可能在其他中断时有多帧数据进入
    {
        ...
        for (size_t i = 0; i < idx; ++i)
        { // 两者相等说明这是要找的实例
            if (_hcan == can_instance[i]->can_handle &&( rxconf.StdId == can_instance[i]->rx_id)&&(rxconf.IDE==CAN_ID_STD))//标准id
            {
            ...
            if(_hcan == can_instance[i]->can_handle&&((rxconf.ExtId>>8)&0xFF)==can_instance[i]->rx_id&&(rxconf.IDE==CAN_ID_EXT))//扩展id
            {
                if (can_instance[i]->can_module_callback != NULL) // 回调函数不为空就调用
                {
                    can_instance[i]->rx_len = rxconf.DLC;                      // 保存接收到的数据长度
